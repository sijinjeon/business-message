...
> **작성일**: 2026-01-01
> **업데이트**: 2026-01-05 - 환율 변환 유틸리티 및 인스턴트 액션 지원 로직 추가
...
### 2.3 환율 및 비용 유틸리티 (`src/utils/exchange.ts`)
- **convertUSDtoKRW**: 최신 환율 정보를 기반으로 USD 비용을 KRW로 변환.
- **formatCurrency**: 통화별 포맷팅 제공 (₩, $).

### 2.4 스토리지 유틸리티 (`src/utils/storage.ts`)
...
- **getStorageData / setStorageData**: `chrome.storage.local` 접근 간소화.

### 2.5 암호화 유틸리티 (`src/utils/crypto.ts`)
