...
### 단축키 기능 개선
- **input/textarea 지원 강화**: `selectionStart/End`를 사용한 직접 텍스트 추출로 호환성 향상
- **React 프레임워크 호환**: 텍스트 교체 후 `input`, `change` 이벤트 발생

### Service Worker 안정성
- 시작 시 등록된 단축키 상태 자동 로깅
- 하트비트 알람 상태 확인 강화

### 에러 핸들링 개선
- 제한된 페이지 감지 (chrome://, edge://, about: 등)
- Content Script 없이도 시스템 알림 표시 가능
- Content Script 동적 주입 시 manifest에서 파일 경로 자동 로드
...
3. [텍스트 조작 및 메시징](./docs/specs/03_클립보드_연동.md)
4. [데이터베이스 및 스토리지 스펙](./docs/specs/06_데이터베이스_스토리지.md)
